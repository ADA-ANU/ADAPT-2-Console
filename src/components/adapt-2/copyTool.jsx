...
export default class CopyTool extends Component {
  ...
  componentDidMount() {
    ...
    this.props.adapt2Store.resetSelection();
  }
  ...
  submit = (form) => {
    ...
    axios
      .post(API_URL.createProdDS, json, {
        timeout: 1400000,
      ...
      .then((data) => {
        console.log(data);
        this.props.systemStore.handleFinalResultOpen(
          {
          ...
          if (err.response.status === 403) {
            ...
            for (let serv of servers) {
              ...
              if (serv.alias === form.destinationServer) {
                ...
                this.props.systemStore.handleFailedAPI(
                  serv.id,
                  2,
                  err.response.data
                );
  ...
  getFileList = async () => {
    ...
    try {
      let check = await this.props.systemStore.getFileListByDOI(
        doi,
        doiMessage,
        userid
      );
        ...
        if (server.indexOf("/") > 0) {
          ...
          if (temp.length === 1) {
            ...
            if (value.indexOf("doi") > 0) {
              ...
              this.props.systemStore.getFileListByDOI(
                doi,
                temp[0].alias,
                userid
              );
        ...
        if (server.indexOf("/") > 0) {
          ...
          if (temp.length === 1) {
            ...
            if (value.indexOf("doi") > 0) {
              ...
              this.props.systemStore.getFileListByDestinationDOI(
                doi,
                temp[0].alias,
                userid
              );
              console.log(temp[0].alias, doi, "EDIT_DS", true);
              this.props.systemStore.checkDSPermission(
                temp[0].alias,
                doi,
                "EDIT_DS",
                true
  ...
  dataverseOnChange = (value, label) => {
    ...
    const dvID = value;
    const dvName = label;
    this.props.systemStore.checkDVPermission(
      this.props.authStore.ctSelectedServer,
      dvID,
      dvName,
      "ADD_DS",
      true
  ...
  render() {
    ...
    const treeData =
      Object.keys(authStore.ctDVList).length > 0 && authStore.ctSelectedServer
        ? authStore.ctDVList[authStore.ctSelectedServer].dataverses
        : [];
    ...
    const props = {
      onRemove: (file) => {
        ...
        this.setState({
          localTargetKeys: localKeys,
          remoteTargetKeys: remoteKeys,
        });
      },
      multiple: true,
      listType: "picture",
      beforeUpload: (file) => {
        ...
        let remoteFileListIncludes =
          datasource.filter((ele) => ele.filename === file.name).length > 0
            ? true
            : false;
    ...
    const submitCheck = () => {
      ...
      if (copyRange !== 3) {
        ...
        if (
          systemStore.doiValid &&
          systemStore.dataversePermissionValid &&
          systemStore.destinationDOIValid &&
          systemStore.datasetPermissionValid
        )
          return false;
        else return true;
      }
    ...
    return (
      <div
        style={{ background: "white", paddingTop: "2%", paddingBottom: "2vh" }}
      >
          ...
          >
            ...
            <Row style={{ marginTop: "2vh", marginBottom: "2vh" }}>
              ...
              <Col
                ...
                xxl={{ span: 3, offset: 1 }}
              >
                <Anchor onClick={this.handleAnchorClick} offsetTop={150}>
                  ...
                  <Link
                    ...
                    title={
                      copyRange !== 3
                        ...
                        : "Existing Destination Dataset"
                    }
                  />
                  {systemStore.showfinalResultDVFiles ? (
                    <Link href="#finalResult" title="Final Result" />
                  ) : null}
              ...
              <Col
                style={{
                  boxShadow:
                    "0 1px 4px rgba(0, 0, 0, 0.1), 0 0 20px rgba(0, 0, 0, 0.1)",
                }}
                xs={{ span: 22, offset: 0 }}
                sm={{ span: 20, offset: 0 }}
                md={{ span: 18, offset: 1 }}
              ...
              >
                <div
                  ...
                  ref={(ref) => {
                    systemStore.copyToolSourceURL = ref;
                  }}
                >
                ...
                <Form.Item
                  ...
                  rules={[
                    {
                      required:
                        (this.state.doiExisting === false &&
                          this.state.serverExisting === false) ||
                ...
                {this.state.server ? (
                  <div style={{ marginLeft: "30%", marginBottom: "1vh" }}>
                    <Tag
                      color="default"
                    ...
                    {this.state.doiMessage ? (
                      <Tag
                        style={{ marginLeft: "1vw" }}
                        color={
                          this.state.doiMessage === "Server not found."
                            ? "#f50"
                            : "#87d068"
                        }
                      >
                ...
                {this.state.doi ? (
                  <div style={{ marginLeft: "30%", marginBottom: "5vh" }}>
                    ...
                    <Tag
                      style={{ marginLeft: "1vw" }}
                      color={systemStore.doiValid ? "#87d068" : "#f50"}
                    >
                      {systemStore.doiValid ? "Valid" : systemStore.doiMessage}
                    </Tag>
                    <Spin
                      spinning={systemStore.isDoiLoading}
                      delay={20}
                      indicator={doiLoadingIcon}
                    />
                  </div>
                ) : null}
                <div id="copyRange">
                    ...
                    >
                      <div style={radioStyle}>
                        <Radio value={1}>
                          Metadata & Files{" "}
                          {this.extraInfo(
                            "creates a new Dataset draft in destination Dataverse"
                          )}
                        </Radio>
                      </div>
                      <div style={radioStyle}>
                      ...
                      <div style={radioStyle}>
                        <Radio value={3}>
                          Files Only{" "}
                          {this.extraInfo(
                            "copies to an existing Destination Dataset"
                          )}
                        </Radio>
                      </div>
                    </Radio.Group>
            ...
            <Row style={{ marginTop: "2vh", marginBottom: "2vh" }}>
              <Col
                style={{
                  boxShadow:
                    "0 1px 4px rgba(0, 0, 0, 0.1), 0 0 40px rgba(0, 0, 0, 0.1)",
                }}
                xs={{ span: 22, offset: 1 }}
                sm={{ span: 20, offset: 2 }}
                md={{ span: 18, offset: 3 }}
                  ...
                  <>
                    ...
                    {this.state.destinationServer ? (
                      <div style={{ marginLeft: "30%", marginBottom: "1vh" }}>
                        ...
                        {this.state.destinationDOIMessage ? (
                          <Tag
                            style={{ marginLeft: "1vw" }}
                            color={
                              this.state.doiMessage === "Server not found."
                    ...
                    {this.state.destinationDOI ? (
                      <div style={{ marginLeft: "30%", marginBottom: "5vh" }}>
                        ...
                        <Tag
                          style={{ marginLeft: "1vw" }}
                          color={
                            systemStore.destinationDOIValid ? "#87d068" : "#f50"
                          }
                        >
                          {systemStore.destinationDOIValid
                            ? "Valid"
                            : systemStore.destinationDOIMessage}
                        </Tag>
                        <Spin
                          spinning={systemStore.isDestinationDoiLoading}
                          delay={20}
                          indicator={doiLoadingIcon}
                        />
                      </div>
              ...
              <Row style={{ marginTop: "2vh", marginBottom: "2vh" }}>
                ...
                >
                  <FinalResult
                    ...
                    destinationDOIOnChange={this.destinationDOIOnChange}
                  />
                </Col>
              </Row>
            ) : null}
