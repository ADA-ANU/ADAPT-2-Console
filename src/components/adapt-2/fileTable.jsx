...
export default class FileTable extends Component {
  ...
  render() {
    ...
    const rowSelectionLocal = {
      selectedRowKeys: [...systemStore.localSelectedKeys.values()]
        .flat()
        .map((ele) => ele.filename),
      // onChange: (selectedRowKeys, selectedRows) => {
      //     console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
    ...
    const rowSelectionRemote = {
      selectedRowKeys: [...systemStore.remoteSelectedKeys.values()]
        .flat()
        .map((ele) => ele.filename),
      // onChange: (selectedRowKeys, selectedRows) => {
      //     console.log(`selectedRowKeys: ${selectedRowKeys}`, 'selectedRows: ', selectedRows);
    ...
    const switchCheck = () => {
      if (adapt2Store.selection === 2) {
        ...
        if (!adapt2Store.createDataset && !systemStore.existingShellDS)
          return true;
        else return false;
      }
    };
              ...
              <Col>
                <div style={{ paddingLeft: "1vw" }}>
                  <Switch
                    ...
                    onChange={(value) =>
                      this.handleSwitchOnChange(value, "local")
                    }
                  />
                </div>
              </Col>
              ...
              >
                ...
                {systemStore.testCheck.length > 0
                  ? systemStore.testCheck.map((option) => (
                      <Checkbox
                        indeterminate={
                          systemStore.localCheckStatus.get(option).indeterminate
                        }
                        onChange={(e) =>
                          systemStore.handleCheckOnchange(
                            e.target.checked,
                            option,
                            "local"
                          )
                        ...
                        checked={
                          systemStore.localCheckStatus.get(option).checked
                        }
                        key={option}
              ...
              <Col>
                <div style={{ paddingLeft: "1vw" }}>
                  <Switch
                    ...
                    defaultChecked={false}
                    onChange={(value) =>
                      this.handleSwitchOnChange(value, "remote")
                    }
              ...
              >
                ...
                {systemStore.testCheck.length > 0
                  ? systemStore.testCheck.map((option) => (
                      <Checkbox
                        indeterminate={
                          systemStore.remoteCheckStatus.get(option)
                            .indeterminate
                        }
                        onChange={(e) =>
                          systemStore.handleCheckOnchange(
                        ...
                        checked={
                          systemStore.remoteCheckStatus.get(option).checked
                        }
                        key={option}
