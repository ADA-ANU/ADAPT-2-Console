...
export class SystemStore {
  ...
  @action resetKeys() {
    let filteredLocalKeys = this.localTargetKeys.filter(
      (ele) => !this.lastFileList.includes(ele)
    );
    let filteredRemoteKeys = this.remoteTargetKeys.filter(
      (ele) => !this.lastFileList.includes(ele)
    );
  ...
  @action popupInputModal(serverAlias) {
    ...
    for (let server of servers) {
      if (server.alias === serverAlias) {
        console.log(server);
        this.handleFailedAPI(
          server.id,
          2,
          `Bad api key, please enter the correct one.`
        );
  ...
  @action deleteFileFromFileList(fileID, filename) {
    ...
    console.log(prefix);
    let newList = this.sortedFileList
      .get(prefix)
    ...
    if (this.localSelectedKeys.get(prefix)) {
      let newKeys = this.localSelectedKeys
        .get(prefix)
        .filter((file) => file.filename !== filename);
      console.log(newKeys);
    ...
    if (this.remoteSelectedKeys.get(prefix)) {
      let newKeys = this.remoteSelectedKeys
        .get(prefix)
        .filter((file) => file.filename !== filename);
      this.remoteSelectedKeys.set(prefix, newKeys);
    }
    const userUploadListTemp = this.userUploadedFiles.filter(
      (ele) => ele.id !== fileID
    );
  ...
  @action updateCheckStatus(prefix, indeterminate, checked, ele) {
    if (ele === "local") {
      this.localCheckStatus.set(prefix, {
        indeterminate: indeterminate,
        checked: checked,
      });
    } else if (ele === "remote") {
      this.remoteCheckStatus.set(prefix, {
        indeterminate: indeterminate,
        checked: checked,
      });
    }
  ...
  @action updateCheckStatusWithNewPrefix(prefix) {
    ...
    if (this.localSelectedKeys.get(prefix)) {
      console.log(
        [...this.sortedFileList.get(prefix)].length,
        [...this.localSelectedKeys.get(prefix)].length
      );
      if (
        [...this.sortedFileList.get(prefix)].length ===
        [...this.localSelectedKeys.get(prefix)].length
    ...
    if (this.remoteSelectedKeys.get(prefix)) {
      if (
        [...this.sortedFileList.get(prefix)].length ===
        [...this.remoteSelectedKeys.get(prefix)].length
      ) {
        this.updateCheckStatus(prefix, false, true, "remote");
      } else {
  ...
  @action isFullDeselected(prefix, ele) {
    if (ele === "local") {
      return (
        !this.localSelectedKeys.get(prefix) ||
        [...this.localSelectedKeys.get(prefix)].length === 0
      );
    } else if (ele === "remote") {
      return (
        !this.remoteSelectedKeys.get(prefix) ||
        [...this.remoteSelectedKeys.get(prefix)].length === 0
      );
    }
  }
  @action localAddRowKey(row, selected) {
    const rowKey = row.filename;
    let prefix = this.regexPrefix.test(rowKey.slice(0, 2))
      ? rowKey.slice(0, 2)
      : "other";
    console.log(prefix);
    if (selected) {
      if (this.localSelectedKeys.has(prefix)) {
        ...
        this.localSelectedKeys.set(prefix, [
          ...this.localSelectedKeys.get(prefix),
          row,
        ]);
      } else {
  ...
  @action remoteAddRowKey(row, selected) {
    ...
    if (selected) {
      if (this.remoteSelectedKeys.has(prefix)) {
        console.log("key found");
        this.remoteSelectedKeys.set(prefix, [
          ...this.remoteSelectedKeys.get(prefix),
          row,
        ]);
      } else {
  ...
  @action resetFileList() {
    ...
    let filteredList = this.fileList.filter(
      (ele) => !this.lastFileList.includes(ele.filename)
    );
  ...
  @action sortFile(file, type) {
    ...
    if (!this.testCheck.includes(prefix)) {
      ...
      this.sortedFileList.set(prefix, [
        file,
        ...this.sortedFileList.get(prefix),
      ]);
    }
    if (type && type === "add") {
      if (this.localSelectedKeys.get(prefix)) {
  ...
  @action getFileListByDOI(doi, server, userid) {
    ...
    return axios
      .post(API_URL.Get_Dataset_FileList_ByDOI, data)
      .then(
        action((res) => {
          ...
          if (err.response) {
            ...
            if (err.response.status === 404) {
              ...
              this.handleAPIInputErrorMsg(
                `${PERMISSION_CATEGORY.VIEW_UNPUBLISHED_DV.errorMsg} in doi:${doi} dataset.`
              );
              this.handleAPIInputModal(true);
              //this.resetKeys()
              //this.resetFileList()
  ...
  @action getDatasetInfoByADAID(adaid, userid) {
    ...
    return axios
      .post(API_URL.getADAFolderInfo, data)
      .then(
        action((res) => {
      ...
      .catch(
        action((err) => {
          ...
          if (err.response) {
            ...
            this.detectDuplicateFiles(
              err.response.data.adaFolderContent,
              this.localTargetKeys
            );
            if (err.response.status === 404) {
              console.log("404");
  ...
  @action checkDVPermission(
    ...
    this.isDoiLoading = true;
    return axios
      .post(API_URL.Check_DV_Permission, data)
      .then(
        action((res) => {
          console.log(res.data);
          if (res.data === false) {
            if (modalOpen) {
              ...
              this.handleAPIInputErrorMsg(
                `${PERMISSION_CATEGORY[permissionType].errorMsg} in ${dvName} dataverse.`
              );
              this.handleAPIInputModal(true);
            } else {
          ...
          if (err.response) {
            if (err.response.status === 401) {
              ...
              this.setCheck(
                serverAlias,
                dvID,
                PERMISSION_CATEGORY[permissionType].type
              );
  ...
  @action checkDSPermission(serverAlias, doi, permissionType, modalOpen) {
    ...
    this.isDoiLoading = true;
    return axios
      .post(API_URL.Check_DS_Permission, data)
      .then(
        action((res) => {
          if (res.data.permission === false) {
            if (modalOpen) {
              ...
              this.datasetPermissionValid = false;
              this.handleAPIInputErrorMsg(
                `${PERMISSION_CATEGORY[permissionType].errorMsg} ${res.data.dsName}.`
              );
              this.handleAPIInputModal(true);
            } else {
              this.datasetPermissionValid = false;
